...
int MaterialDraw(const S_BOARD *pos) {
  ...
  if (!pos->pceNum[wR] && !pos->pceNum[bR] && !pos->pceNum[wQ] &&
      !pos->pceNum[bQ]) {
...
#define ENDGAME_MAT \
  (1 * PieceVal[wR] + 2 * PieceVal[wN] + 2 * PieceVal[wP] + PieceVal[wK])
