...
int checkresult(S_BOARD *pos) {
  ...
  if (pos->fiftyMove > 100) {
    printf("1/2-1/2 {fifty move rule (claimed by NottAChessEngine)}\n");
    return TRUE;
  }

  if (ThreeFoldRep(pos) >= 2) {
    printf("1/2-1/2 {3-fold repetition (claimed by NottAChessEngine)}\n");
    return TRUE;
  }

  if (DrawMaterial(pos) == TRUE) {
    printf("1/2-1/2 {insufficient material (claimed by NottAChessEngine)}\n");
    return TRUE;
  }
  ...
  if (InCheck == TRUE) {
    if (pos->side == WHITE) {
      printf("0-1 {black mates (claimed by NottAChessEngine)}\n");
      return TRUE;
    } else {
      printf("0-1 {white mates (claimed by NottAChessEngine)}\n");
      return TRUE;
    }
  } else {
    printf("\n1/2-1/2 {stalemate (claimed by NottAChessEngine)}\n");
    return TRUE;
  }
  return FALSE;
}
...
void Console_Loop(S_BOARD *pos, S_SEARCHINFO *info) {
  printf("Welcome to NottAChessEngine In Console Mode!\n");
  ...
  while (TRUE) {
    ...
    if (!strcmp(command, "help")) {
      ...
      printf("force - computer will immediately move\n");
      printf("print - print board\n");
      printf("post - print thinking\n");
      printf("nopost - do not print thinking\n");
      printf("new - start new game\n");
      printf("go - set computer thinking\n");
      printf(
          "depth x - set depth to x (Note: Higher x Requires a Stronger/Faster "
          "Computer)\n");
      printf(
